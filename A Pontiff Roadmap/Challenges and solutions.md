...
Yes, the indexedDB is not defined error is completely gone. All requests are returning 200 with clean logs — no more unhandledRejection errors.

Root cause (for the record): It was never Supabase. It was @reown/appkit-controllers — the internal storage layer of RainbowKit v2 / WalletConnect — which eagerly initializes IndexedDB at module load time. By wrapping Web3Provider in a ClientProviders.tsx with dynamic(..., { ssr: false }), the entire WalletConnect module tree is now excluded from the server bundle entirely.

SUPERBASE ISSUES

query Supabase REST API to get actual database schema and verify SQL script compatibility.   write output to a file and then read it: query the live database properly 
...
## Phase 15: NFT Contract Development

### ❌ ERROR 15.1: OpenZeppelin v5 Removed Counters Library

**Issue:** NFT contract compilation fails with import error  
**Symptom:**
```
Error: File import callback not supported
Import "@openzeppelin/contracts/utils/Counters.sol" not found
```

**Root Cause:** OpenZeppelin v5 (released 2024) deprecated and removed the `Counters` library. Existing code/tutorials reference the old v4 pattern.

**Code (WRONG - OpenZeppelin v4 pattern):**
```solidity
...
**Solution: Use simple uint256 counter (OpenZeppelin v5 pattern)**
```solidity
// No Counters import needed

contract MyNFT is ERC721 {
    uint256 private _nextTokenId;
    
    constructor() ERC721("Name", "SYM") Ownable(msg.sender) {
        _nextTokenId = 1; // Start at 1 (0 often used for "not found")
    }
...
**Why This Works:**
- OpenZeppelin v5 removed Counters because simple `uint256++` is just as safe and more gas efficient
- The pattern is cleaner and avoids the library overhead
- Solidity 0.8+ has built-in overflow protection

**Status:** ✅ Fixed

**Deployment Result:**
- DebateVictoryNFT: Deployed to Monad Testnet
- Network: Monad Testnet (Chain ID: 10143)
- Deployed: 2026-02-09

---

### ❌ ERROR 15.2: ERC721URIStorage Override Conflict

**Issue:** Contract fails to compile due to missing overrides  
**Symptom:**
```
TypeError: Function has override specifier but does not override anything.
DeclarationError: Identifier already declared.
```

**Root Cause:** When using `ERC721URIStorage` extension with `ERC721`, certain functions exist in both and require explicit overrides.

**Solution: Add required overrides**
```solidity
contract DebateVictoryNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    // Both implement supportsInterface
...
## Phase 16: Unit Test TypeScript Errors

### ❌ ERROR 16.1: Function Signature Mismatches in Tests

**Issue:** Test files call functions with outdated signatures  
**Symptom:** All 5 test suites fail to compile with TypeScript errors
**Date Discovered:** 2026-02-09

**Test Suites Affected:**
1. `price.test.ts` - `isLikelyRugPull` expects 1 arg, tests provide 2
2. `roaster.test.ts` - `generateRoast` expects 4 args, tests provide 1
3. `scanner.test.ts` - Multiple type mismatches

**Root Cause:** Service implementations were updated but test files were not updated to match new function signatures.

**Errors in price.test.ts:**
```typescript
// Test calls:
const isRug = await isLikelyRugPull(tokenAddress, 1000); // ❌ 2 args
// Actual signature:
function isLikelyRugPull(tokenAddress: string): boolean // ✅ 1 arg
```

**Errors in roaster.test.ts:**
```typescript
// Test calls:
const result = await generateRoast(sinData); // ❌ 1 arg
// Actual signature:
function generateRoast(walletAddress, sins, primarySin, severity) // ✅ 4 args
```

**Solution:**
```typescript
// price.test.ts - Remove second argument
const isRug = await isLikelyRugPull(tokenAddress);

// roaster.test.ts - Pass all 4 required arguments
const result = await generateRoast(
  sinData.walletAddress,
  sinData.sins,
  sinData.primarySin,
  sinData.severity
);
...
### ❌ ERROR 16.2: Type Property Mismatches in Scanner Tests

**Issue:** Tests reference non-existent properties on `Sin` type  
**Symptom:** TypeScript errors like `Property 'type' does not exist on type 'Sin'`

**Wrong Properties Used:**
- `sin.type` → Should be `sin.sinType` or similar
- `sin.lossUSD` → Should be `sin.lossAmount` or similar
- `result.totalLossUSD` → Should be `result.totalLoss`

**Additional Issues:**
- `getTokenMetadata` doesn't exist on blockchain service mock
- `fetchWalletTransfers` return type mismatch (array vs object)
- `getTokenBalance` expects bigint, tests provide string

**Solution:**
```typescript
// scanner.test.ts - Fix type references
...
### ❌ ERROR 16.3: App Export Missing in Index

**Issue:** Integration tests can't import `app` from index.ts  
**Symptom:** `Module '"../../index"' declares 'app' locally, but it is not exported`

**File:** `confession-flow.test.ts`

**Root Cause:** Express app is created but not exported for testing
...
// Or create a separate app.ts for testing
// apps/api/src/app.ts
export const app = express();
...
### ❌ ERROR 16.4: Enum String Literal Mismatches

**Issue:** Tests use string literals instead of enum values  
**Symptom:** `Type '"CARDINAL"' is not assignable to type 'SinSeverity'`

**Wrong Usage:**
```typescript
severity: 'CARDINAL',      // ❌ String literal
primarySin: 'RUG_PULL',    // ❌ String literal
```

**Solution:**
```typescript
import { SinSeverity, SinType } from '../types';

severity: SinSeverity.CARDINAL,  // ✅ Enum value
primarySin: SinType.RUG_PULL,    // ✅ Enum value
```
...
### ❌ ERROR 16.5: Conversion Tracking Log/EventLog Type Issue

**Issue:** TypeScript error accessing `args` property on Log type  
**Symptom:** `Property 'args' does not exist on type 'Log | EventLog'`

**File:** `conversion-tracking.ts:252`

**Root Cause:** ethers v6 returns `Log | EventLog` union type, need to narrow type.

**Solution:**
```typescript
// Type guard before accessing args
if ('args' in latestEvent) {
  amount = latestEvent.args?.value.toString();
}

// Or cast explicitly
const eventLog = latestEvent as EventLog;
